...
gofmt -w main.go
GOOS=windows GOARCH=amd64 CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc \
  go build -mod=vendor -ldflags="-H windowsgui" -o 인수인계관리_JC02_v1.4.32.exe .
```
...
### 설비별 PM 체크리스트 (v1.4.30 도입, v1.4.32 상시 표 방식으로 변경)
설비마다 PM(예방정비) 시 할 일을 적어두는 표. 메모처럼 **로컬 파일 저장**이라 껐다
켜도 유지된다.
- **위치/형태 (v1.4.32)**: 메인 목록(`.left`)과 달력(`.right`) **사이**의 독립 칼럼
